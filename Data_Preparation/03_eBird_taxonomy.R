...
# ID sp. Taxonomy ---------------------------------------------------------------
ID_sp <- read.csv("Data/eBird/Obs_species/ID_filt_all.csv") %>% select(-X)

# IUCN
IUCN_all <- read.csv("Data/IUCN/Taxonomy_Sept2024.csv")

ID_sp <- ID_sp %>% mutate(IUCN_name = case_when(scientific_name == "Aegithalos exilis" ~ "Psaltria exilis",
...
match <- left_join(ID_sp, IUCN_sub, by = c("IUCN_name" = "scientificName"))
match %>% filter(is.na(redlistCategory))

write.csv(match, "Data/Taxonomy/eBird_IUCN_ID_match.csv")

# AVONET taxonomy

avonet_bl_raw <- read.csv("Data/Traits/AVONET/AVONET_Birdlife_Taxonomoy_RAW.csv")
...
test <- ID_avo_sub %>% left_join(avonet_taxo2, by = c("AVONET_BL_name" = "Species1"))
test %>% filter(is.na(Mass))

write.csv(ID_avo_sub, "Data/Taxonomy/eBird_IUCN_AVONET_ID_match.csv")
